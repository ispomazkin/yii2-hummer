...
namespace ispomazkin\hummer\controllers;
...
class HummerController extends Controller
{
    /**
     * int
    ...
    {
        $module = Yii::$app->getModule('hummer');
        /* @var $module Module*/
        if (!$module)
            throw new \Exception('The module Hummer was not configured in your app config file');

        if (!($this->api_host = $module->host))
            throw new \Exception('The property Host of Hummer module was not configured in your app config file');

        if (!($this->api_image_path = $module->image_path))
            throw new \Exception('The property Image_path of Hummer module was not configured in your app config file');

        if (!($this->search_pattern = $module->search_pattern))
            throw new \Exception('The property search_pattern of Hummer module was not configured in your app config file');
        ...
        Yii::setAlias('@hummer_views', Yii::getAlias('@ispomazkin/hummer/views/hummer'))	;
    ...
    {
        $url = $this->api_host.'/hummer/get-years';
        ...
        return $this->render('@hummer_views/index',[
            'data'=>$new,
        ]);
    }
    ...
    protected function createSeo($type,$data)
    {
        ...
        switch($type)
        {
            ...
            case 'categories':
                $this->view->params['breadcrumbs'][] =  ['label'=>$this->titlePattern['years'],'url'=>Url::to(['hummer/index'])];
                break;

            case 'groups':
                $this->view->params['breadcrumbs'][] =  ['label'=>$this->titlePattern['years'],'url'=>Url::to(['hummer/index'])];
                $this->view->params['breadcrumbs'][] =  ['label'=>$this->titlePattern['categories'],'url'=>Url::to(['hummer/categories',
            ...
            case 'parts' :
                ...
                $this->view->params['breadcrumbs'][] = [
                    ...
                    'url' => Url::to(['hummer/groups','year_url'=>$data['model_url'],'category_url'=>$data['category_url'],
                    ])
                ];
    ...
    {
        $url = $this->api_host.'/hummer/get-categories?url='.$year_url;
        $data = Json::decode($this->sendRequest($url));
        ...
        return $this->render('@hummer_views/categories',[
            'data'=>$data['body'],
        ]);
    }
    ...
    {
        $url = $this->api_host.'/hummer/get-groups?year_url='.$year_url.'&category_url='.$category_url;
        ...
        return $this->render('@hummer_views/groups_last',[
            'data'=>$data['body'],
    ...
    {
        $url = $this->api_host.'/hummer/get-parts?year_url='.$year_url.'&last_url='.$last_url;
        ...
        return $this->render('@hummer_views/parts',[
            'data'=>$data['body'],
            'img_path'=>$this->api_host . $this->api_image_path,
